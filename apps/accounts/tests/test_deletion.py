...
def _delete(client, payload):
    return client.delete(DELETE_URL, data=payload, content_type="application/json")


class TestDeletionAuth:
    def test_requires_authentication(self, client):
        response = _delete(client, {"password": VALID_PASSWORD})
        assert response.status_code == 401

    def test_wrong_password_generic_403(self, client, user):
        client.force_authenticate(user)
        response = _delete(client, {"password": "WrongPassword123!"})
...
class TestDeletionSuccess:
    def test_delete_returns_204(self, client, user):
        client.force_authenticate(user)
        response = _delete(client, {"password": VALID_PASSWORD})
        assert response.status_code == 204

    def test_row_retained_and_anonymized(self, client, user):
        client.force_authenticate(user)
        _delete(client, {"password": VALID_PASSWORD})
    ...
    def test_all_refresh_tokens_blacklisted(self, client, user):
        login = client.post(
    ...
    def test_cannot_login_after_deletion(self, client, user):
        client.force_authenticate(user)
        _delete(client, {"password": VALID_PASSWORD})
    ...
    def test_deleted_email_never_leaks_original(self, client, user):
        client.force_authenticate(user)
        _delete(client, {"password": VALID_PASSWORD})
        user.refresh_from_db()
